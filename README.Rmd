...
algorithms for approximate analytics on data too large to hold or process
exactly. Sketches are compact, mergeable summaries that answer queries such
as approximate distinct counts, quantiles and ranks, frequent items and
...
**Tuple sketches**, a Theta-extension that pairs per-key value arrays with
...
- `ebpps()` -- EBPPS (Exact and Bounded Probabilistic Proportional-to-Size), a
...
Build a KLL sketch from a numeric vector and query approximate quantiles and
ranks:
...
Sketches are mergeable, so partial sketches built from different chunks of a
stream can be combined into one:
...
Sketches can be serialized to a raw vector and restored, for interoperability
with other Apache DataSketches implementations:

```{r serialize, results = "hide"}
