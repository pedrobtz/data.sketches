# KLL float quantile sketch: the R6 implementation and its exported
# constructor. The generator is internal; users go through `kll_floats()`.
# See `_dev/WORKING-ON.md` for the public API contract and implementation
# status. This mirrors `R/quantiles-kll-doubles.R`; the only behavioral
# difference is that retained items are stored as native `float` (32-bit),
# so values round-trip with reduced precision.
# Native bridge functions live in `src/kll_floats.cpp`.
...
kll_floats_sketch_generator <- R6Class(
  "kll_floats_sketch",
  cloneable = FALSE,
  public = list(
    ...
    update = function(x) {
      ...
      kll_floats_update_cpp(private$ptr, x)
      invisible(self)
    },
    ...
    merge = function(other) {
      check_kll_floats(other, "other")
      if (identical(private$ptr, kll_floats_ptr(other))) {
        abort_invalid(
          "A sketch cannot be merged into itself.",
          "datasketches_self_merge"
        )
      }
      kll_floats_merge_cpp(private$ptr, kll_floats_ptr(other))
      invisible(self)
    },
    ...
    rank = function(x, inclusive = TRUE) {
      ...
      ok <- !is.na(x)
      if (any(ok)) {
        out[ok] <- kll_floats_get_ranks_cpp(private$ptr, x[ok], inclusive)
      }
      out
    },
    ...
    n = function() {
      kll_floats_get_n_cpp(private$ptr)
    },

    k = function() {
      kll_floats_get_k_cpp(private$ptr)
    },

    num_retained = function() {
      kll_floats_get_num_retained_cpp(private$ptr)
    },

    is_empty = function() {
      kll_floats_is_empty_cpp(private$ptr)
    },

    is_estimation_mode = function() {
      kll_floats_is_estimation_mode_cpp(private$ptr)
    },
    ...
    rank_error = function(pmf = FALSE) {
      ...
      kll_floats_rank_error_cpp(private$ptr, pmf)
    },
    ...
    inspect = function(levels = TRUE, items = FALSE) {
      ...
      cat(kll_floats_to_string_cpp(private$ptr, levels, items))
      invisible(self)
    },

    serialize = function() {
      kll_floats_serialize_cpp(private$ptr)
    },
...
#'   reconstruct.
#'
#' @return A `kll_floats_sketch` object. Key methods:
#'   \describe{
...
#'
#' @export
kll_floats <- function(x = NULL, k = NULL, bytes = NULL) {
  kll_floats_sketch_generator$new(x = x, k = k, bytes = bytes)
}
