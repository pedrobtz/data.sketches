# Tests for the KLL floats public surface. Mirrors
# test-quantiles-kll-doubles.R; values round-trip through 32-bit float, so
# exact-equality checks use whole numbers that are exactly representable.

test_that("kll_floats() builds an empty sketch of the default width", {
  s <- kll_floats()
  expect_s3_class(s, "kll_floats_sketch")
...
test_that("kll_floats(k =) sets the width and validates the range", {
  expect_equal(kll_floats(k = 64)$k(), 64L)

  expect_error(kll_floats(k = 2), class = "datasketches_invalid_k")
  expect_error(kll_floats(k = 70000), class = "datasketches_invalid_k")
  expect_error(kll_floats(k = 100.5), class = "datasketches_invalid_k")
  expect_error(kll_floats(k = c(200, 300)), class = "datasketches_invalid_k")
})

test_that("kll_floats(x =) updates the sketch and reports exact stats below k", {
  s <- kll_floats(as.double(1:100))
...
test_that("metadata accessors agree with summary()", {
  ...
  expect_equal(info$type, "kll_floats")
...
test_that("rank_error() reflects k and the pmf flag", {
  loose <- kll_floats(k = 50)$rank_error()
  tight <- kll_floats(k = 400)$rank_error()
  expect_true(tight < loose)
  expect_error(
    kll_floats()$rank_error(pmf = NA),
    class = "datasketches_invalid_flag"
  )
})

test_that("rank_error() reflects effective k after a mixed-width merge", {
  tight <- kll_floats(as.double(1:10000), k = 400)
  loose <- kll_floats(as.double(10001:20000), k = 50)
...
test_that("merge() combines sketches and mutates the receiver", {
  a <- kll_floats(as.double(1:500))
  b <- kll_floats(as.double(501:1000))
...
test_that("merge() rejects a non-sketch argument", {
  ...
  fake <- structure(new.env(), class = "kll_floats_sketch")
...
test_that("format() and as.character() give the same concise representation", {
  ...
  expect_match(format(s), "^<kll_floats_sketch\\[n=100, k=200\\]>$")
})

test_that("print() returns the sketch invisibly", {
  s <- kll_floats(as.double(1:100))
  expect_output(out <- print(s), "kll_floats_sketch")
...
test_that("serialize() round-trips through the bytes constructor", {
  ...
  restored <- kll_floats(bytes = bytes)
...
test_that("constructor exclusivity is enforced", {
  bytes <- kll_floats(as.double(1:100))$serialize()
  expect_error(
    kll_floats(x = 1:10, bytes = bytes),
    class = "datasketches_invalid_args"
  )
  expect_error(
    kll_floats(bytes = bytes, k = 200),
    class = "datasketches_invalid_args"
  )
  expect_error(
    kll_floats(bytes = "not raw"),
    class = "datasketches_invalid_args"
  )
})
